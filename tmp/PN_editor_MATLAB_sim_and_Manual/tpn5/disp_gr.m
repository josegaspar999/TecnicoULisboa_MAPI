...
% function [XX]=disp_gr(A, RM)
%
% Input parameters this function are matrix A and matrix RM, which are
%   the output parameters of function reach_graph.m
%
% Output parameter this function is matrix XX, which  contains these items:
%   state, parent, subs, trans, Xi,Yi, Xf,Yf, col
%
	...
	XX(:,item)=[column_of_subs(j) parent number_subs_of_state trans Xi Xf Yi Yf col]';
      ...
      if (Yi>1)  			% counting of coordinates of returning point ( we will go through this point)
       ...
       max_yy=max(max_Yi,max_Yf);   
	    plot([Xi Xf+1/2 Xf],[Yi max_yy+1 Yf],'k-.');
        text(Xf+1/2,max_yy+1,transit)
       else plot([Xi 1 Xf],[Yi -3 Yf],'k-.');text(1,-3,transit);     
      end;
