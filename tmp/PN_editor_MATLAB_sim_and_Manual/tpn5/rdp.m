...
if length(filename)>4 && strcmp(upper(filename(end-3:end)), '.XML')
    % use another reader for filenames not ending .RDP but ending as .XML
    [Pre,Post,M0] = rdp_pipe2(filename);
	return
end
